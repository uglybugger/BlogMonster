...
{
    public class EmbeddedResourceBlogPostRepository : IRepository<BlogPost>
    {
        ...
        public EmbeddedResourceBlogPostRepository(IEmbeddedResourceImagePathMapper imagePathMapper, IMarkDownTransformer markDownTransformer)
        {
            ...
            _blogPosts = manifestResourceNames
        ...
        {
            return query.Filter(_blogPosts.AsQueryable());
        }
        ...
        private BlogPost TryLoadBlogPost(string resourceName)
        {
            ...
            string title;
            var overrideTitleResourceName = "{0}.Title.txt".FormatWith(string.Join(".", tokens.Skip(3).Take(4).ToArray()));
            ...
            var markdownWithImagesRemapped = _imagePathMapper.ReMapImagePaths(markdown, string.Join(".", tokens.Take(7).ToArray()));
